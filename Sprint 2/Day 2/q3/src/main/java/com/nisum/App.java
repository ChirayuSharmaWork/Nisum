...
public class App {
    public static void main(String[] args) {
        ...
        String searchString = "rOy";

        List<Employee> result = employeeMap.values().stream().flatMap(List::stream).filter(e -> e.getFirstName().toLowerCase().contains(searchString.toLowerCase()) ||
                e.getLastName().toLowerCase().contains(searchString.toLowerCase())).collect(Collectors.toList());
